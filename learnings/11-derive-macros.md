...
| Derive | `#[derive(Foo)]` on a type | Reads the type, emits an `impl Foo for ThatType` block. Examples: `Debug`, `Serialize`. |
...
Some derives ship with the compiler/stdlib; others come from crates.

| Always available | Require a crate |
|---|---|
...
- `#[derive(X)]` is syntactic sugar for "please generate an `impl X for ThisType`."
- `X` must be either a built-in derive (`Debug`, `Clone`, etc.) or a proc-macro from a crate.
